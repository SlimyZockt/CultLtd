...
SteamCtx :: struct {
	using platform_info: PlatformInfo,
	// lobby_size:          u16,
	// max_lobby_size:      u16,
	flags:               SteamFlags,
	user:                ^steam.IUser,
...
steam_init :: proc(ctx: ^SteamCtx) {
	...
	ctx.player_id = PlayerId(steam.User_GetSteamID(ctx.user))
	ctx.platform_features = {.Multiplayer, .Achievements}
...
steam_update_callback :: proc(
	...
	for (steam.ManualDispatch_GetNextCallback(h_pipe, &callback)) {
		if callback.iCallback == .SteamAPICallCompleted {
			...
			#partial switch callback.iCallback {} 	// "new" steam api
		...
		case .SteamNetConnectionStatusChangedCallback:
			...
			if is_host && can_accept {
				...
				if res != .OK {
					...
					steam_disconnect_current_player_from_all(ctx)
				}
			...
			if has_error {
				...
				steam_disconnect_current_player_from_all(ctx)
				if is_host {
					network_event_callback(NetEventPeerConnected{PlayerId(remote_id)}, user_data)
				} else {
					network_event_callback(
						NetEventPeerDisconnected{PlayerId(remote_id)},
						user_data,
					)
				}
			}
			...
			if !is_host && data.eOldState == .FindingRoute && info.eState == .Connected {
				network_event_callback(NetEventConnectedToHost{PlayerId(remote_id)}, user_data)
			}
		...
		case .LobbyEnter:
			...
			ctx.socket = steam.NetworkingSockets_CreateListenSocketP2P(
				ctx.network_sockets,
				0,
				0,
				nil,
			...
			if PlayerId(lobby_owner) == ctx.player_id { 	// HOST
				if ctx.lobby_size <= 1 {
					network_event_callback(NetEventCreated{}, user_data)
		...
		// network_event_callback(NetEventConnectedToHost{nil}, user_data)
		}
...
steam_disconnect_current_player_from_all :: proc(ctx: ^SteamCtx) {
	ctx.flags -= {.Host}
...
steam_deinit :: proc(ctx: ^SteamCtx) {
	steam_disconnect_current_player_from_all(ctx)
...
steam_create_lobby :: proc(ctx: ^SteamCtx, max_lobby_size: u16 = 8) {
	ctx.flags += {.Host}
...
steam_send :: proc(ctx: ^SteamCtx, data: ^$T, size: u32) {
	switch conn in ctx.connection {
	case ConnectionToHost:
