...
main :: proc() {
	...
	if monitor_count > 1 {
		for i in 0 ..= monitor_count {
			...
			monitor_id = rl.GetMonitorHeight(i) > rl.GetMonitorHeight(i + 1) ? i : i + 1
		}
		rl.SetWindowMonitor(monitor_id)
	}

	render_rect: [2]f32 = {f32(rl.GetRenderWidth()), f32(rl.GetRenderHeight())}

	player: Entity
	player.speed = 500
	player.size = {32, 64}
	p_camera: rl.Camera2D
	p_camera.target = player.pos
	p_camera.offset = render_rect / 2
	p_camera.zoom = 1
	...
	for !rl.WindowShouldClose() {
		...
		for elapsed_time >= LOGIC_TICK_RATE {
			...
			if input.x != 0 || input.y != 0 {
				...
				player.pos += dir * player.speed * LOGIC_TICK_RATE
			}
		}

		p_camera.target = player.pos + (player.size / 2)
